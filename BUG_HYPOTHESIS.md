...
- **stacked_params_mapping** — `("in_proj_qkvz", "in_proj_qkv", (0,1,2))` and `("in_proj_qkvz", "in_proj_z", 3)` find correct parameters. `("in_proj_ba", "in_proj_b", 0)`, `("in_proj_ba", "in_proj_a", 1)` likewise. (`vllm/model_executor/models/qwen3_5.py:280-294`)
...
3. `stacked_params_mapping`: `in_proj_qkv` → `in_proj_qkvz` shard `(0,1,2)` (Q+K+V block)
4. `stacked_params_mapping`: `in_proj_z` → `in_proj_qkvz` shard `3` (Z)
...
- `vllm/model_executor/models/qwen3_5.py:280-294` — `stacked_params_mapping` for Qwen3.5
