...
class Qwen3_5Model(Qwen3NextModel):
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        ...
        def _preprocess(w_iter: Iterable[tuple[str, torch.Tensor]]):
            for name, weight in w_iter:
                # GGUF stores shared_expert_gate as 1D; param expects [1, H].
                if "mlp.shared_expert_gate" in name and weight.ndim == 1:
                    weight = weight[None, :]
                yield name, weight

        loader = AutoWeightsLoader(self)
        return loader.load_weights(_preprocess(weights), mapper=mapper)
